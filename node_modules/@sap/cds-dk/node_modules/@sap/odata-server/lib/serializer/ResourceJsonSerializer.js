...
class ResourceJsonSerializer {
        ...
        try {
            ...
            if (value === null || value === undefined) {
                ...
                Object.assign(result,
                    this._serializeStructuralProperties(type, null, value,
                        this._getSelectedPaths(selectItems, expandItems), expandItems, true),
        ...
        try {
            ...
            result.value = value.map(valueItem => {
                ...
                return valueItem === null ?
                    this._serializeNullValue(propertyOrReturnType) :
                    Object.assign({},
                        this._serializeStructuralProperties(type, null, valueItem, selectedPaths, expandItems, true),
    ...
    _serializeEntity(entityTypeOrSet, data, selectedPaths, expandItems, eTagRequired, isKeyRequired) {
        ...
        let serializedAnnotations = (entitySet && entitySet.isConcurrent() || eTagRequired) && !isTransient ?
            this._serializeAnnotations([MetaProperties.ETAG], data, MetaProperties.ETAG) :
            {};
        ...
        return Object.assign(serializedAnnotations, serializedOperationAdvertisements,
    ...
    _reduceExpandItems(expandItems, name) {
        return expandItems
            // Only expand items with isAll() or with paths starting with the complex-property name are interesting.
            && expandItems.filter(item => item.isAll()
                || item.getPathSegments().length > 1 && item.getPathSegments()[0].getPathSegmentIdentifier() === name)
                ...
                .map(item => {
                    ...
                    newItem.setOption('complexPath', (item.getOption('complexPath') || []).concat(name));
                    return newItem;
                });
    }
     ...
     * @private
     */
    _serializeStructuralProperties(type, entitySetOrSingleton, data, selectedPaths, expandItems, isKeyRequired) {
        if (typeof data !== 'object') throw new SerializationError('Value of a structured property must be an object');
        ...
        for (const [propertyName, property] of type.getProperties()) {
            if (selectedPaths.length === 0
                ...
                serializedStructuralProps[propertyName] =
                    this._serializeProperty(property, entitySetOrSingleton, propertyValue, paths, reducedExpandItems,
                        isKeyRequired);
            }
        }
    ...
    _serializeNavigationProperties(type, entitySetOrSingleton, data, expandItems, isKeyRequired) {
        if (!(typeof data === 'object')) throw new SerializationError('Value of entity data must be an object');
        ...
        for (const [navPropertyName, navProperty] of type.getNavigationProperties()) {
            ...
            if (expandItem) {
                ...
                const expandedEntitySet = entitySetOrSingleton ?
                    entitySetOrSingleton.getRelatedBindingTarget(
                        (expandItem.getOption('complexPath') || []).concat(navPropertyName).join('/')) :
                    null;
     ...
     * @private
     */
    _serializeProperty(property, entitySetOrSingleton, propertyValue, selectedPaths, expandItems, isKeyRequired) {
        if (propertyValue === null || propertyValue === undefined) {
        ...
        try {
            return property.isCollection() ?
                ...
                this._serializePropertyValue(property, entitySetOrSingleton, propertyValue, selectedPaths, expandItems,
                    isKeyRequired);
        } catch (error) {
     ...
     * @private
     */
    _serializePropertyValue(property, entitySetOrSingleton, propertyValue, selectedPaths, expandItems, isKeyRequired) {
        ...
        switch (propertyType.getKind()) {
            ...
            case EdmTypeKind.COMPLEX:
                return Object.assign(
                    this._serializeStructuralProperties(propertyType, entitySetOrSingleton, propertyValue,
                        selectedPaths, expandItems, isKeyRequired),
                    this._serializeNavigationProperties(propertyType, entitySetOrSingleton, propertyValue, expandItems,
                        isKeyRequired));
            default:
                throw new SerializationError(
