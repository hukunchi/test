...
class Extend extends BaseCommand {
    ...
    static async storeFiles(content, targetDirectory, filter) {
        ...
        for (const [filepath, content] of [...filesToWrite.entries()]) {
            ...
            Extend._logWrittenFile(path.relative(process.cwd(), normalizedFilepath));
        }
        return writtenFiles;
    }
    ...
    static async run(url, { directory, subdomain, passcode, username, password, wsdl, templates, force }) {
        ...
        try {
            ...
            try {
                ...
                const authOptions = await Extend.determineAuthOptions(projectSettings, {
                    passcode,
                    username,
                    password
                });

                await Extend.runExtend(projectSettings.url, projectSettings.folder, authOptions, runOnExistingProject, { wsdl, templates });
            } catch (error) {
                await Extend.handleHttpError(error, projectFolder, projectSettings.url);
            }
        } catch (mainError) {
    ...
    static async runExtend(url, projectFolder, authOptions, runOnExistingProject, { wsdl, templates }) {

        const extendUrl = Extend.getMtxApiUrl(url, COMMAND_URL_SUFFIX);
        ...
        const { body: [ response ] } = await got(extendUrl, options);
        ...
        if (wsdl) {
            ...
            const customFileResponse = await got(customFileUrl, options);
